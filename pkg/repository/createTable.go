...
func (db *CreatorTable) Paramtypes(tableName string, i interface{}) bool {
	logger := logging.GetLogger()
	query := fmt.Sprintf("CREATE TABLE `%s` (`id` INT NOT NULL , `enddate` DATE NULL DEFAULT NULL , `startdate` DATE NULL DEFAULT NULL , `updatedate` DATE NULL DEFAULT NULL , `isactive` INT NOT NULL DEFAULT '1' , `code` TEXT NULL DEFAULT NULL , `description` VARCHAR(255) NOT NULL , `name` VARCHAR(255) NOT NULL , `text` VARCHAR(512) NOT NULL, PRIMARY KEY (`id`)) ENGINE = InnoDB;", tableName)
	_, err := db.db.Exec(query)
	if err != nil {
		logger.Info(query)
		logger.Error(err)
