...
func (db *CreatorTable) ObjectDivision(tableName string, i model_objectAddr.ITEMS) bool {
	logger := logging.GetLogger()
	query := fmt.Sprintf("CREATE TABLE `%s`(`id` INT NOT NULL,`object_id` VARCHAR(255) NULL DEFAULT NULL,`parent_id` VARCHAR(255) NULL DEFAULT NULL,`child_id` VARCHAR(255) NULL DEFAULT NULL,`change_id` INT(255) NULL DEFAULT NULL, `text` VARCHAR(255) NULL DEFAULT NULL) ENGINE = InnoDB;;", tableName)
	_, err := db.db.Exec(query)
	if err != nil {
		logger.Info(query)
		logger.Error(err)
...
func (db *CreatorTable) ChangeHistory(tableName string, i model_other.CHANGEHISTORY) bool {
	logger := logging.GetLogger()
	query := fmt.Sprintf("CREATE TABLE `%s`(`id` INT NULL,`object_id` VARCHAR(255) NULL DEFAULT NULL,`adr_object_id` VARCHAR(255) NULL DEFAULT NULL,`change_id` INT(255) NULL DEFAULT NULL,`oper_type_id` INT(20) NULL DEFAULT NULL,`change_date` DATE NULL DEFAULT NULL,`ndoc_id` INT(255) NULL DEFAULT NULL,`text` VARCHAR(255) NULL DEFAULT NULL) ENGINE = InnoDB;;", tableName)
	_, err := db.db.Exec(query)
	if err != nil {
		logger.Info(query)
		logger.Error(err)
...
func (db *CreatorTable) ReestrObject(tableName string, i model_other.REESTROBJECTS) bool {
	logger := logging.GetLogger()
	query := fmt.Sprintf("CREATE TABLE `%s`(`id` INT NULL DEFAULT NULL,`object_id` VARCHAR(255) NULL DEFAULT NULL,`object_gid` VARCHAR(255) NULL DEFAULT NULL,`change_id` INT(255) NULL DEFAULT NULL,`level_id` INT(20) NULL DEFAULT NULL,`update_date` DATE NULL DEFAULT NULL,`create_date` DATE NULL DEFAULT NULL,`text` VARCHAR(255) NULL DEFAULT NULL) ENGINE = InnoDB;", tableName)
	_, err := db.db.Exec(query)
	if err != nil {
		logger.Info(query)
		logger.Error(err)
...
func (db *CreatorTable) Ndoctypes(tableName string, i model.NDOCTYPES) bool {
	logger := logging.GetLogger()
	query := fmt.Sprintf("CREATE TABLE `%s` (`id` INT NOT NULL  , `enddate` DATE NULL DEFAULT NULL , `startdate` DATE NULL DEFAULT NULL , `name` VARCHAR(255) NULL DEFAULT NULL , `text` VARCHAR(255) NULL DEFAULT NULL, PRIMARY KEY (`id`)) ENGINE = InnoDB;", tableName)
	_, err := db.db.Exec(query)
	if err != nil {
		logger.Info(query)
		logger.Error(err)
...
func (db *CreatorTable) Ndockinds(tableName string, i model.NDOCKINDS) bool {
	logger := logging.GetLogger()
	query := fmt.Sprintf("CREATE TABLE `%s` (`id` INT NOT NULL , `name` VARCHAR(255) NULL DEFAULT NULL , `text` VARCHAR(255) NULL DEFAULT NULL, PRIMARY KEY (`id`)) ENGINE = InnoDB;", tableName)
	_, err := db.db.Exec(query)
	if err != nil {
		logger.Info(query)
		logger.Error(err)
...
// General
func (db *CreatorTable) Params(tableName string, i model.PARAMS) bool {
	logger := logging.GetLogger()
	query := fmt.Sprintf("CREATE TABLE `%s` (`id` INT NOT NULL , `object_id` VARCHAR(255) NULL DEFAULT NULL , `change_id` VARCHAR(255) NULL DEFAULT NULL , `change_id_end` VARCHAR(255) NULL DEFAULT NULL , `type_id` INT NULL DEFAULT NULL , `value` VARCHAR(255) NULL DEFAULT NULL , `update_date` DATE NULL DEFAULT NULL , `start_date` DATE NULL DEFAULT NULL , `end_date` DATE NULL DEFAULT NULL, PRIMARY KEY (`id`)) ENGINE = InnoDB;", tableName)
	_, err := db.db.Exec(query)
	if err != nil {
		logger.Info(query)
		logger.Error(err)
